...
impl Preprocessor {
    pub fn new(file_path: &str) -> Self {
        ...
        fs::copy(file_path, preprocessed_file_path);
        Preprocessor {
    ...
    fn include_file(&mut self, filename: &str) {
        for include_path in &self.include_paths {
            ...
            if full_path.exists() {
                ...
                self.new_file.write_at(included_contents.unwrap().as_bytes(), self.pos_in_file);
            }
        }
    }
    ...
    pub fn process_file(&mut self, file_path: &str) {
        ...
        let file_to_preprocess = File::open(file_path).expect("Should have been able to open the file");
